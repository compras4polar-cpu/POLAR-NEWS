...
async function fetchBrent() {
  ...
  if (!apiKey) {
    const similarNames = Object.keys(process.env).filter((k) => k.toUpperCase().includes("EIA"));
    return {
      id: "brent",
      unavailable: true,
      ...
      debugSimilarEnvNames: similarNames // não expõe valores, só nomes — remover depois do diagnóstico
    };
  }
